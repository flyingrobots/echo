...
fn run_suite(manifest_path: PathBuf) -> Result<()> {
    ...
    for entry in manifest {
        ...
        if golden_path.exists() {
             ...
             let expected: echo_dind_harness::dind::Golden = serde_json::from_reader(BufReader::new(f_golden))?;
             assert_eq!(hashes, expected.hashes_hex, "Hash mismatch for {}", entry.path);
        }
    }
