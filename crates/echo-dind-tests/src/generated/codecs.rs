...
#![allow(unused_mut, unused_variables, unused_assignments, unused_imports, clippy::let_and_return)]
...
pub struct MotionV2View<'a> { raw: &'a [u8] }
impl<'a> MotionV2View<'a> {
    #[inline] fn slot_i64_le(&self, slot: usize) -> i64 {
        let start = slot * 8;
        let chunk: [u8; 8] = self.raw[start..start + 8].try_into().unwrap();
        i64::from_le_bytes(chunk)
    }
    pub fn pos_raw(&self) -> [i64; 3] { [self.slot_i64_le(0), self.slot_i64_le(1), self.slot_i64_le(2)] }
    pub fn vel_raw(&self) -> [i64; 3] { [self.slot_i64_le(3), self.slot_i64_le(4), self.slot_i64_le(5)] }
}
impl<'a> AtomView<'a> for MotionV2View<'a> {
    const TYPE_ID: TypeId = TYPEID_PAYLOAD_MOTION_V2;
    const BYTE_LEN: usize = 48;
    #[inline] fn parse(bytes: &'a [u8]) -> Option<Self> { Some(Self { raw: bytes }) }
}
pub struct MotionV2Builder { buf: [u8; 48] }
impl MotionV2Builder {
    ...
    #[inline] pub fn into_bytes(self) -> bytes::Bytes { bytes::Bytes::copy_from_slice(&self.buf) }
}
...
pub mod ops {
    ...
    pub mod route_push {
        ...
        pub fn decode_vars(bytes: &[u8]) -> Option<Args> {
            ...
            offset += 4;
            if s_len > super::super::MAX_STRING_BYTES || bytes.len() < offset + s_len { return None; }
            let path = std::str::from_utf8(&bytes[offset..offset+s_len]).ok()?.to_string();
            offset += s_len;
            Some(Args { path })
        }
    }
    ...
    pub mod toast {
        ...
        pub fn decode_vars(bytes: &[u8]) -> Option<Args> {
            ...
            offset += 4;
            if s_len > super::super::MAX_STRING_BYTES || bytes.len() < offset + s_len { return None; }
            let message = std::str::from_utf8(&bytes[offset..offset+s_len]).ok()?.to_string();
            offset += s_len;
            Some(Args { message })
        }
    }
    ...
    pub mod put_kv {
        ...
        pub fn decode_vars(bytes: &[u8]) -> Option<Args> {
            ...
            offset += 4;
            if k_len > super::super::MAX_STRING_BYTES || bytes.len() < offset + k_len { return None; }
            let key = std::str::from_utf8(&bytes[offset..offset+k_len]).ok()?.to_string();
            offset += k_len;
            if bytes.len() < offset + 4 { return None; }
            let v_len = u32::from_le_bytes(bytes[offset..offset+4].try_into().unwrap()) as usize;
            offset += 4;
            if v_len > super::super::MAX_STRING_BYTES || bytes.len() < offset + v_len { return None; }
            let value = std::str::from_utf8(&bytes[offset..offset+v_len]).ok()?.to_string();
            offset += v_len;
            Some(Args { key, value })
        }
    }
