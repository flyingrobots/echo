...
use super::{
    array_field, as_map, as_text, canonical_sorted_map, canonical_text, invalid_artifact,
    map_field, resource_ref_value, text_field, validate_binding, validate_target_profile, Artifact,
    BoundArtifact, Diagnostic, DiagnosticSeverity, DigestAlgorithm, ProviderRefusalKind,
    ProviderRefusalV1, SemanticInput, SemanticInputKind, VerificationOutputArtifact,
    VerificationOutputKind, VerificationRequestV1, VerificationSuccessV1,
};
...
pub(super) fn is_requested(request: &VerificationRequestV1) -> bool {
    request
        .requested_outputs
        .iter()
        .any(|output| output.role == REPORT_ROLE || output.domain == REPORT_DOMAIN)
        || request.semantic_inputs.iter().any(|input| {
            input.kind == SemanticInputKind::Auxiliary("executable-operation-package".to_owned())
        })
}
...
pub(super) fn verify(
    ...
    let target_ir = validate_bound(&request.target_ir, TARGET_IR_DOMAIN)?;
    let closure = select_closure(&request.semantic_inputs)?;

    let adapter = validate_bound(&closure.adapter.artifact, ADAPTER_DOMAIN)?;
    let package = validate_bound(&closure.package.artifact, PACKAGE_DOMAIN)?;
    ...
    let (outcome, diagnostics, diagnostic_bytes) = if actual == expected {
        ("accepted", Vec::new(), Vec::new())
    } else {
        let diagnostic = Diagnostic {
            code: "echo.verifier.executable-operation-package-mismatch".to_owned(),
            severity: DiagnosticSeverity::Error,
            message:
                "Executable package does not preserve the compiler and target-adapter relation"
                    .to_owned(),
            repair: None,
        };
        (
            "rejected",
            vec![diagnostic],
            b"echo.verifier.executable-operation-package-mismatch".to_vec(),
        )
    };
...
fn verify_compiler_produced_pure(
    request: &VerificationRequestV1,
    ...
    let (outcome, diagnostics, diagnostic_bytes) = if actual == expected {
        ...
        let diagnostic = Diagnostic {
            code: "echo.verifier.executable-operation-package-mismatch".to_owned(),
            severity: DiagnosticSeverity::Error,
            message: "Executable package does not preserve the compiler and pure target relation"
                .to_owned(),
            repair: None,
        };
        (
...
fn validate_pure_lawpack(
    ...
    _request: &VerificationRequestV1,
...
fn validate_pure_target_ir(
    ...
    if request.target_ir.reference.coordinate != TARGET_IR_COORDINATE
...
fn encode_expected_pure_package(
    request: &VerificationRequestV1,
    closure: &ClosureInputs<'_>,
    intent_name: &str,
    intent: &CanonicalValueV1,
    ...
    let program = canonical_map([
        ...
        (
            "target_ir_artifact",
            CanonicalValueV1::Bytes(request.target_ir.artifact.bytes.clone()),
        ),
    ]);
    let program = encode_canonical_cbor_v1(&program)
        .map_err(|_| invalid_artifact(PACKAGE_COORDINATE, "pure program could not be encoded"))?;
    ...
    encode_canonical_cbor_v1(&package)
        .map_err(|_| invalid_artifact(PACKAGE_COORDINATE, "pure package could not be encoded"))
}

fn validate_requested_output(request: &VerificationRequestV1) -> Result<(), ProviderRefusalV1> {
    let [output] = request.requested_outputs.as_slice() else {
        return Err(unsupported_output(REPORT_ROLE));
    };
    if output.role != REPORT_ROLE
        || output.kind != VerificationOutputKind::VerifierReport
        || output.domain != REPORT_DOMAIN
    {
...
fn unsupported_output(subject: &str) -> ProviderRefusalV1 {
    super::refusal(
        ...
        "executable-operation verification emits only the generic relation report",
    )
}
...
fn select_closure(inputs: &[SemanticInput]) -> Result<ClosureInputs<'_>, ProviderRefusalV1> {
    ...
    Ok(ClosureInputs {
        ...
        package: unique_input(
            inputs,
            &SemanticInputKind::Auxiliary("executable-operation-package".to_owned()),
        )?,
...
fn validate_exports(
    ...
    let effect = required_array(value, "effects", "exports.echo-operation")?
        .iter()
    ...
    let (failure_name, _) = single_text_map_entry(required_map(
        effect,
        "effectFailures",
...
fn validate_lawpack(
    ...
    let selected = required_array(value, "targetAdapters", "lawpack.echo-operation")?
        .iter()
        .find(|candidate| {
...
fn validate_adapter(
    ...
    let implementation = required_map_field(
        required_map(value, "effectImplementations", "adapter.echo-operation")?,
        semantic_effect,
    ...
    if text_field(
        required_map(implementation, "failureMappings", "adapter.echo-operation")?,
        intent.failure_name,
    ) != Some(PRECONDITION_MISMATCH)
    {
        return Err(super::unsupported_semantics("adapter.echo-operation"));
    }
    Ok(())
...
fn semantic_lawpack_member_coordinate(
    ...
    let expected_digest = super::digest_review(&lawpack.artifact.reference.digest)
        .ok_or_else(|| invalid_artifact(&lawpack.role, "lawpack digest is malformed"))?;
    ...
    *nodes = nodes
    ...
    match required_text(value, "kind", subject)? {
        "record" => {
            ...
            let entries = as_map(required_map(value, "fields", subject)?)
                .ok_or_else(|| invalid_artifact(subject, "projection fields must be a map"))?;
...
fn encode_expected_package(
    ...
    let program_bytes = encode_canonical_cbor_v1(&program).map_err(|_| {
        invalid_artifact(PACKAGE_COORDINATE, "expected program could not be encoded")
    })?;
    let core_identity = hash_from_bound(&request.core)?;
    let semantic_closure = canonical_map([
        (
    ...
    encode_canonical_cbor_v1(&package)
        .map_err(|_| invalid_artifact(PACKAGE_COORDINATE, "expected package could not be encoded"))
}

fn build_report(
    package: &super::ResourceRef,
    target_ir: &super::ResourceRef,
    result_projection: &super::ResourceRef,
    outcome: &str,
    ...
    let report = canonical_sorted_map([
        ...
        (
            ...
            resource_ref_value(result_projection).map_err(|()| {
                invalid_artifact(
                    REPORT_ROLE,
                    "result projection reference could not be encoded",
                )
            })?,
        ),
        ...
        (
            "package",
            resource_ref_value(package).map_err(|()| {
                invalid_artifact(REPORT_ROLE, "package reference could not be encoded")
            })?,
        ),
        (
            "targetIr",
            resource_ref_value(target_ir).map_err(|()| {
                invalid_artifact(REPORT_ROLE, "Target IR reference could not be encoded")
            })?,
        ),
        ...
        (
            "diagnosticAbi",
            resource_ref_value(&super::ResourceRef {
                coordinate: super::DIAGNOSTIC_ABI.coordinate.to_owned(),
                digest: super::Digest {
                    algorithm: DigestAlgorithm::Sha256,
                    bytes: super::DIAGNOSTIC_ABI.framed_sha256.to_vec(),
                },
            })
            .map_err(|()| {
                invalid_artifact(REPORT_ROLE, "diagnostic ABI reference could not be encoded")
            })?,
        ),
        ("diagnosticBytes", CanonicalValueV1::Bytes(diagnostic_bytes)),
    ])
    .map_err(|()| invalid_artifact(REPORT_ROLE, "report map could not be constructed"))?;
    encode_canonical_cbor_v1(&report)
        .map_err(|_| invalid_artifact(REPORT_ROLE, "report could not be canonically encoded"))
}
...
fn canonical_map<const N: usize>(entries: [(&str, CanonicalValueV1); N]) -> CanonicalValueV1 {
    CanonicalValueV1::Map(
        entries
            .into_iter()
