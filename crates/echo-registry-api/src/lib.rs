...
pub const SEMANTIC_OPERATION_ID_LAW_V1: &str = "echo.semantic-operation-id.fnv1-32/v1";

/// Echo operation id reserved for privileged scheduler-control envelopes.
pub const RESERVED_CONTROL_OPERATION_ID: u32 = u32::MAX;

/// Echo operation id reserved for witnessed-suffix-import envelopes.
pub const RESERVED_IMPORT_SUFFIX_OPERATION_ID: u32 = u32::MAX - 1;

/// Returns whether an operation id is reserved for an Echo protocol envelope.
///
/// Application contracts and generated semantic operations must not use these
/// ids. Echo-owned privileged paths remain responsible for constructing the
/// corresponding protocol envelopes.
#[must_use]
pub const fn is_reserved_operation_id(operation_id: u32) -> bool {
    operation_id == RESERVED_CONTROL_OPERATION_ID
        || operation_id == RESERVED_IMPORT_SUFFIX_OPERATION_ID
}

/// Derives a persisted Echo operation id from an exact semantic coordinate.
///
/// The byte preimage is the UTF-8 law coordinate, one zero delimiter, the
/// generic kind byte (`0` for queries and `1` for mutations), and the exact
/// UTF-8 semantic coordinate. FNV-1 32-bit processes that complete preimage.
/// This law is deliberately domain-separated from the legacy Wesley derivation
/// over GraphQL field names.
///
/// Generators remain responsible for refusing Echo protocol-reserved ids and
/// for detecting collisions across the complete generated operation set. They
/// must not salt, probe, or rename coordinates to repair a collision.
#[must_use]
pub const fn stable_semantic_operation_id_v1(kind: OpKind, semantic_coordinate: &str) -> u32 {
    let mut hash = 2_166_136_261_u32;
    let law = SEMANTIC_OPERATION_ID_LAW_V1.as_bytes();
    let mut index = 0;
    while index < law.len() {
        hash = semantic_operation_id_fnv1_step(hash, law[index]);
        index += 1;
    }
    hash = semantic_operation_id_fnv1_step(hash, 0);
    hash = semantic_operation_id_fnv1_step(
        hash,
        match kind {
            OpKind::Query => 0,
            OpKind::Mutation => 1,
    ...
    let coordinate = semantic_coordinate.as_bytes();
    index = 0;
    while index < coordinate.len() {
        hash = semantic_operation_id_fnv1_step(hash, coordinate[index]);
        index += 1;
    }
    hash
}

const fn semantic_operation_id_fnv1_step(hash: u32, byte: u8) -> u32 {
