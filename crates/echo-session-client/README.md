...
- Offers blocking helpers for tools:
  - `connect_channels(path)` and `connect_channels_for(path, rmg_id)` connect
    to the hub, perform a handshake + RMG subscription, and spawn a background
    thread that decodes packets into:
    - `RmgFrame` snapshots/diffs,
    - `Notification` values,
...
- Exposes the `tool` module for hexagonal tool architectures:
  - `tool::SessionPort` trait: abstract interface for draining notifications
    and RMG frames and clearing RMG streams.
...
- Used by `rmg-viewer` and future tools to keep domain logic dependent only on
...
- See the Tool hexagon pattern and crate map in
  ...
  and the RMG Viewer section (`08-rmg-viewer-spec.tex`) for a worked example
