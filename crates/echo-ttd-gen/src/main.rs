...
fn generate_channels(ir: &TtdIR) -> TokenStream {
    ...
    for ch in &ir.channels {
        let name = &ch.name;
        let const_name = format_ident!("CHANNEL_{}", name.to_uppercase());
...
fn generate_rules(ir: &TtdIR) -> TokenStream {
    ...
    tokens.extend(quote! {
        ...
        pub fn rules_for_op(op_name: &str) -> impl Iterator<Item = &'static RuleInfo> {
            RULES.iter().filter(move |r| r.op_name == op_name)
        }

        pub fn rules_from_state(state: &str) -> impl Iterator<Item = &'static RuleInfo> {
            RULES.iter().filter(move |r| r.from_states.contains(&state))
        }
    });
