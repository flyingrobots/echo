...
//! Contract tests for Edict canonical values and admitted CDDL roots.
...
use echo_wesley_gen::provider_contract_pack::{
    admit_provider_contract_pack_v1, ProviderContractValidationErrorKind,
};
...
fn typed_digest(byte: u8) -> CanonicalValueV1 {
    CanonicalValueV1::Array(vec![
        text("sha256"),
        CanonicalValueV1::Bytes(vec![byte; 32]),
    ])
}

fn authority_facts() -> CanonicalValueV1 {
    string_map([
        ("apiVersion", text("edict.authority-facts/v1")),
        (
            "source",
            string_map([
                ("kind", text("lawpack")),
                ("coordinate", text("echo.dpo-lawpack@1")),
                ("digest", typed_digest(0x11)),
            ]),
        ),
        ("operationProfiles", map([])),
        ("effectWriteClasses", map([])),
        ("budgets", map([])),
    ])
}
...
#[test]
fn admitted_resources_are_exact_canonical_byte_and_digest_oracles() {
    ...
    for resource in pack.resources() {
        let decoded = decode_canonical_cbor_v1(resource.canonical_bytes())
            .expect("admitted resource uses canonical CBOR");
        assert_eq!(
            encode_canonical_cbor_v1(&decoded).expect("decoded resource re-encodes"),
            resource.canonical_bytes()
        );
        assert_eq!(
            digest_canonical_value_v1(resource.coordinate(), &decoded)
                .expect("resource domain digest computes"),
...
#[test]
fn contract_admission_requires_canonical_bytes_and_the_owning_root() {
    let pack = admit_provider_contract_pack_v1(CONTRACT_CDDL, CONTRACT_MANIFEST)
        .expect("checked Edict provider contract pack is admitted");
    let value = authority_facts();
    let bytes = encode_canonical_cbor_v1(&value).expect("authority facts encode");

    let admitted = pack
        .validate_contract_bytes("authority-facts", &bytes)
        .expect("authority facts satisfy their admitted root");
    assert_eq!(
        encode_canonical_cbor_v1(&admitted).expect("admitted value re-encodes"),
        bytes
    );
    assert_eq!(
        pack.validate_contract_bytes("echo-owned", &[0x18, 0x00])
            .expect_err("unknown contract rejects before byte decoding")
            .kind(),
        ProviderContractValidationErrorKind::UnknownContract
    );
    let canonical_error = pack
        .validate_contract_bytes("authority-facts", &[0x18, 0x00])
        .expect_err("noncanonical bytes reject before schema admission");
    assert_eq!(canonical_error.subject(), "authority-facts");
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(CanonicalValueErrorKind::NonCanonical)
    );
    let null = encode_canonical_cbor_v1(&CanonicalValueV1::Null).expect("null encodes");
    let schema_error = pack
        .validate_contract_bytes("authority-facts", &null)
        .expect_err("decoding is not owning-root admission");
    assert_eq!(
        schema_error.kind(),
        ProviderContractValidationErrorKind::SchemaMismatch
    );
    assert_eq!(schema_error.canonical_value_kind(), None);
    assert_eq!(
        pack.validate_contract_bytes("core-module", &bytes)
            .expect_err("a value is admitted only through its owning root")
            .kind(),
        ProviderContractValidationErrorKind::SchemaMismatch
    );
}
