...
/// via [`warp_engine_free`] to avoid leaking memory.
#[no_mangle]
pub unsafe extern "C" fn warp_engine_new() -> *mut WarpEngine {
    Box::into_raw(Box::new(WarpEngine {
        inner: build_motion_demo_engine(),
    }))
}

/// Releases the engine allocation created by [`warp_engine_new`].
///
/// # Safety
/// `engine` must be a pointer previously returned by [`warp_engine_new`] that
...
#[no_mangle]
pub unsafe extern "C" fn warp_engine_spawn_motion_entity(
...
///
/// # Safety
/// `engine` must be a valid pointer created by [`warp_engine_new`].
#[no_mangle]
...
#[no_mangle]
pub unsafe extern "C" fn warp_engine_apply_motion(
    engine: *mut WarpEngine,
    tx: warp_tx_id,
    node_handle: *const warp_node_id,
) -> bool {
...
#[no_mangle]
pub unsafe extern "C" fn warp_engine_read_motion(
    engine: *mut WarpEngine,
    node_handle: *const warp_node_id,
...
fn handle_to_node_id(handle: *const warp_node_id) -> Option<NodeId> {
...
mod tests {
    ...
    unsafe fn spawn(engine: *mut WarpEngine, label: &str) -> warp_node_id {
        let c_label = CString::new(label).unwrap();
        let mut handle = warp_node_id { bytes: [0; 32] };
        let ok = unsafe {
            warp_engine_spawn_motion_entity(
                engine,
                c_label.as_ptr(),
                1.0,
                2.0,
    ...
    fn ffi_motion_rewrite_is_deterministic() {
        unsafe {
            let engine_a = warp_engine_new();
            let handle_a = spawn(engine_a, "entity-ffi");
            let tx_a = warp_engine_begin(engine_a);
            assert!(warp_engine_apply_motion(
                engine_a,
                tx_a,
                &handle_a as *const _
            ));
            let mut snap_a = warp_snapshot { hash: [0; 32] };
            assert!(warp_engine_commit(engine_a, tx_a, &mut snap_a as *mut _));

            let engine_b = warp_engine_new();
            let handle_b = spawn(engine_b, "entity-ffi");
            let tx_b = warp_engine_begin(engine_b);
            assert!(warp_engine_apply_motion(
                engine_b,
                tx_b,
                &handle_b as *const _
            ));
            let mut snap_b = warp_snapshot { hash: [0; 32] };
            assert!(warp_engine_commit(engine_b, tx_b, &mut snap_b as *mut _));

            assert_eq!(snap_a.hash, snap_b.hash);

            let mut position = [0f32; 3];
            let mut velocity = [0f32; 3];
            assert!(warp_engine_read_motion(
                engine_a,
                &handle_a as *const _,
                position.as_mut_ptr(),
                velocity.as_mut_ptr()
            ...
            warp_engine_free(engine_a);
            warp_engine_free(engine_b);
        }
    }
    ...
    fn ffi_apply_no_match_returns_false() {
        unsafe {
            let engine = warp_engine_new();
            let tx = warp_engine_begin(engine);
            let bogus = warp_node_id { bytes: [1; 32] };
            assert!(!warp_engine_apply_motion(engine, tx, &bogus as *const _));
            warp_engine_free(engine);
        }
    }
}
