...
impl WasmEngine {
    ...
    /// Returns the sequential history of all committed ticks as raw CBOR bytes.
    #[wasm_bindgen]
    pub fn get_ledger_bytes(&self) -> Uint8Array {
        ...
        // Use echo-wasm-abi for deterministic CBOR encoding
        match echo_wasm_abi::encode_cbor(&serializable) {
            Ok(bytes) => Uint8Array::from(bytes.as_slice()),
            Err(_) => Uint8Array::new_with_length(0),
        }
    }

    /// Returns the canonical state hash for the current graph.
    #[wasm_bindgen]
    pub fn state_hash(&self) -> Uint8Array {
        let engine = self.inner.borrow();
