...
\chapter{Parallel Execution and Storage}
\label{chap:boaw}
...
The parallel execution architecture replaces the spike-era monolithic in-place mutation model with immutable snapshots, COW overlays, and lockless parallel execution. The result: deterministic convergence across platforms, massive parallel throughput without locks, and zero-copy snapshot IO.

\section{The Parallel Execution Stack}
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Parallel tick pipeline: ingress $\rightarrow$ plan $\rightarrow$ admit $\rightarrow$ parallel execute $\rightarrow$ merge $\rightarrow$ commit.}
...
This is the determinism drill sergeant for parallel execution. If this invariant fails, a footprint bug exists.

\section{FootprintGuard: Storage Access Enforcement}

In debug builds (or when \texttt{footprint\_enforce\_release} is enabled),
unless \texttt{unsafe\_graph} is set, the system validates both read and
write footprints:

\textbf{Read enforcement:} \texttt{GraphView::new\_guarded()} wraps every
storage read with a footprint check. Attempts to access nodes, edges, or
attachments not declared in the footprint's read sets trigger a
\texttt{FootprintViolation} panic.

\textbf{Write enforcement:} After the executor runs (inside a
\texttt{catch\_unwind} boundary), every emitted op is validated via
\texttt{check\_op()} against the declared write sets. Write violations are
detected at validation time, not at the point of emission. This catches
undeclared writes, cross-warp emissions, and adjacency violations.

Both checks ensure that actual accesses are a subset of the declared sets
(actual $\subseteq$ declared), but they do \emph{not} detect or prevent
over-declaration. The guard enforces safety, not tightness.

The guard adds zero overhead in release builds---it is entirely
\texttt{cfg}-gated and does not appear in the compiled binary unless
explicitly opted-in.
