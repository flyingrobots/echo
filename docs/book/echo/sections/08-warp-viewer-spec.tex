...
\section{Echo WARP Viewer: State Machine \& UI}
\label{sec:warp-viewer}

The WARP Viewer is a thin, rendering-first client. It connects to the Echo Session Host, subscribes to one or more WARP streams, and presents a minimal HUD over a 3D scene. This section fixes the viewer's states, transitions, and on-screen layout so the implementation and the design stay in lockstep.

At the protocol level, the Viewer is ``just'' a client app in the WARP
streaming pipeline. It follows the generic snapshot/diff consumer contract in
Section~\ref{sec:warp-stream-consumers} and sits in the ``Client App'' row of
Table~\ref{tab:warp-role-summary}; what is specific here is \emph{how} it
projects those WARPs into a 3D ring and UI.
...
\begin{itemize}
  ...
  \item Multiple WARPs shown on a ring around the camera; arrow keys rotate the ring to bring one WARP to the front.
  \item Menu overlay offers: Settings, Publish Local WARP, Subscribe to WARP, Back.
\end{itemize}
...
\begin{description}
  ...
  \item[Viewer] 3D scene + HUD. Overlays (Menu, Settings, WARP Directory) sit atop this state.
...
\begin{figure}[htbp]
  ...
  \caption{Top-level WARP Viewer state machine.}
\end{figure}
...
\begin{itemize}
  ...
  \item Requesting WARP directory\ldots
  \item WARP indexes received.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Menu} button (top-left) opens overlay: Settings / Publish Local WARP / Subscribe to WARP / Back.
  \item \textbf{Toasts} stack (top-right).
  \item \textbf{Perf} block (bottom-left) showing FPS/timings (toggled in Settings).
  \item \textbf{Controls} block (bottom-left) showing WASD/QE, mouse look, right-drag spin, wheel zoom, arrow-keys cycle WARPs.
  \item \textbf{Stats} block (bottom-center/right) showing WARP id/epoch, subscription status.
  \item \textbf{Watermark/version} always bottom-right.
\end{itemize}
Overlay logic: Menu, Settings, and WARP Directory sit atop Viewer; closing them restores the HUD.

\subsection{WARP Ring Layout}
WARPs (local + subscribed) are placed on a horizontal ring of radius $R$ around the camera. For $n$ WARPs, positions are
\[
  x_i = R\cos \theta_i, \quad z_i = R\sin \theta_i, \quad \theta_i = \frac{2\pi i}{n} + \theta_\text{offset}.
\]
Arrow keys adjust $\theta_\text{offset}$ to rotate the ring so a chosen WARP sits in front of the camera. Adding subscriptions increases $n$ and redistributes positions evenly.

\begin{figure}[htbp]
  \centering
\begin{tikzpicture}[
    camera/.style={regular polygon,regular polygon sides=3,draw,fill=gray!20,minimum size=0.8cm},
    warp/.style={circle,draw,fill=blue!10,minimum size=0.55cm,font=\scriptsize}
  ]
    \def\R{2.4}
    \node[camera, shape border rotate=90, label={[font=\scriptsize]above:camera}] (cam) at (0,2.8) {};
    \draw[thick,dashed] (0,0) circle (\R);
    \foreach \i/\name in {0/{WARP A},1/{WARP B},2/{WARP C}} {
      \coordinate (p\i) at ({\R*cos(90-120*\i)},{\R*sin(90-120*\i)});
    }
    \node[warp,label={[font=\scriptsize]below:{WARP A}}] at (p0) {};
    \node[warp,label={[font=\scriptsize]below:{WARP B}}] at (p1) {};
    \node[warp,label={[font=\scriptsize]below:{WARP C}}] at (p2) {};
    \draw[->, thick] (cam.south) -- (0,1.7) node[midway,right,font=\scriptsize]{view};
    \draw[very thick, red] (p0) circle [radius=0.32];
  \end{tikzpicture}
  \caption{Overhead ring layout; arrow keys rotate which WARP is active.}
\end{figure}

\subsection{Publish / Subscribe semantics}
\begin{itemize}
  \item \textbf{Publish Local WARP}: register as the sole producer for a chosen WarpId and stream Snapshot \textrightarrow{} gapless Diff to the host.
  \item \textbf{Subscribe to WARP}: open the WARP Directory overlay (listing host-known WarpIds); selecting one sends SubscribeWarp and adds that WARP to the ring; arrow keys cycle which WARP is in front.
\end{itemize}
...
\begin{itemize}
  ...
  \item All overlays reuse the same settings form (host/port, warp id, HUD toggles) to avoid divergent UIs.
\end{itemize}
