...
The RMG Viewer is a concrete example of this architecture, and can serve as a
...
\begin{description}
  ...
  \item[SessionPort] Receive RMG snapshots/diffs and notifications from the
    session hub.
  \item[RenderPort] Request redraws from whatever windowing/surface system is
    in use.
\end{description}

Figure~\ref{fig:tool-hexagon} shows these ports and their adapters for the RMG
Viewer.
...
\begin{figure}[h]
  ...
  \node[adapter, above=of rendport] (winadapt) {Winit + wgpu\\\scriptsize rmg-viewer};
  ...
  \end{tikzpicture}
  \caption{Tool hexagon pattern: core + ports + adapters (RMG Viewer example).}
  \label{fig:tool-hexagon}
\end{figure}

\subsection{RMG Viewer as Reference Implementation}

For the RMG Viewer specifically (Section~\ref{sec:rmg-viewer}):
  ...
  \item \textbf{SessionPort}:
    \begin{itemize}
      ...
      \item Implemented by \texttt{ChannelSession}, which wraps the
            \texttt{RmgFrame} / \texttt{Notification} channels returned by
...
\begin{itemize}
  ...
  \item Use the RMG Viewer layout in this part of the book as a worked example,
        and the JS-ABI/RMG streaming specs
        (Sections~\ref{sec:js-abi-wire} and \ref{sec:rmg-stream-consumers})
  ...
  \begin{tabular}{@{}p{0.24\textwidth}p{0.68\textwidth}@{}}
    ...
    \texttt{echo-session-proto}
      & Logical wire schema for the Echo session hub: \texttt{Message},
        ...
        \texttt{Notification}/\texttt{RmgStream} types that wrap
        \texttt{echo-graph} RMG frames. \\
    \texttt{echo-session-service}
      & Headless session hub process: listens on a Unix socket, handles
        handshakes, tracks per-\texttt{RmgId} stream state, enforces gapless
    ...
    \texttt{rmg-viewer}
      & 3D RMG viewer built on the above ports and adapters: uses
