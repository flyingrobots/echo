...
Section~\ref{sec:rmg-stream-consumers} and in more detail for tools in
Section~\ref{sec:session-service} (Session Service) and
Section~\ref{sec:rmg-viewer} (RMG Viewer) in the Editor Tools volume.
...
\begin{verbatim}
op-envelope = {
  "op": tstr,   ; operation name (e.g. "handshake", "error", "rmg_stream")
...
RMG stream, \dots). The \texttt{ts} field is the kernel logical timestamp; the
