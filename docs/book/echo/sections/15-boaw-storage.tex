...
unless \texttt{unsafe\_graph} is set,
\texttt{GraphView::new\_guarded()} wraps every storage read with a footprint
check. Attempts to access nodes, edges, or attachments not declared in the
footprint's read sets trigger a \texttt{FootprintViolation} panic. This
ensures that actual accesses are a subset of the declared read sets
(actual $\subseteq$ declared), but it does \emph{not} detect or prevent
