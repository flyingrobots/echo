...
\begin{itemize}
    ...
    \item \texttt{patch\_digest} --- canonical hash of merged ops
    \item \texttt{commit\_hash} = $H(\text{parents} \parallel \text{state\_root} \parallel \text{patch\_digest} \parallel \text{schema} \parallel \text{tick} \parallel \text{policy})$
\end{itemize}
