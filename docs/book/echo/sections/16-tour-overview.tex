...
\begin{enumerate}
    ...
    \item \textbf{Parallel Execution} --- Workers execute in parallel
...
\midrule
Engine core & \texttt{crates/echo-app-core/src/engine.rs} \\
Transaction handling & \texttt{crates/echo-app-core/src/transaction.rs} \\
Rule matching & \texttt{crates/warp-core/src/rule.rs} \\
Scheduler (Radix) & \texttt{crates/warp-core/src/scheduler.rs} \\
Parallel execution & \texttt{crates/warp-core/src/parallel/exec.rs} \\
Shard routing & \texttt{crates/warp-core/src/parallel/shard.rs} \\
Delta/merge & \texttt{crates/warp-core/src/tick\_delta.rs} \\
Hashing & \texttt{crates/warp-core/src/snapshot.rs} \\
\bottomrule
\end{tabular}
...
\begin{protip}
Keep a terminal open with the codebase. When you see a file reference like \texttt{src/parallel/exec.rs:61-83}, jump to that location and read the actual code alongside this tour.
\end{protip}
