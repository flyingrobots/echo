...
\begin{quote}
\textbf{Note:} ``BOAW'' is a retired codename. This subsystem now lives under
\texttt{warp\_core::parallel}. The chapter title is preserved for historical
continuity.
\end{quote}

\textbf{Entry Point:} \texttt{execute\_parallel()} \\
\textbf{File:} \texttt{crates/warp-core/src/parallel/exec.rs:61-83}

\begin{bigpicture}
Parallel execution is where Echo's determinism meets parallelism. The key insight: \textbf{execution order doesn't matter if we sort the outputs}.
...
\textbf{File:} \texttt{crates/warp-core/src/parallel/exec.rs:19-35}
...
\section{FootprintGuard Enforcement}

When footprint enforcement is active, each \texttt{ExecItem} is executed via
\texttt{execute\_item\_enforced()} rather than a bare function-pointer call.
This adds two layers of runtime validation:

\begin{enumerate}
    \item \textbf{Read enforcement}: \texttt{GraphView::new\_guarded()} wraps
    the read-only view with an intercepting layer. Every accessor
    (\texttt{node()}, \texttt{edges\_from()}, etc.) checks the footprint's
    declared read sets. Undeclared reads trigger a
    \texttt{FootprintViolation} panic.

    \item \textbf{Write enforcement}: After the executor runs (inside a
    \texttt{catch\_unwind} boundary), \texttt{check\_op()} validates every
    newly-emitted op against the footprint's write sets. This catches
    undeclared writes, cross-warp emissions, adjacency violations (edge ops
    whose \texttt{from} node is not in \texttt{n\_write}), and unauthorized
    instance ops.
\end{enumerate}

\textbf{ExecItemKind} gates instance-level ops: only \texttt{System} items
(internal engine code) may emit \texttt{UpsertWarpInstance},
\texttt{DeleteWarpInstance}, or \texttt{OpenPortal}. User-authored rules
are always \texttt{ExecItemKind::User}.

The guard is \texttt{cfg}-gated: active in debug builds or with the
\texttt{footprint\_enforce\_release} feature; disabled by the
\texttt{unsafe\_graph} escape hatch. Zero overhead applies only when these
checks are disabled.

\begin{protip}
If a footprint violation fires, the error message includes the offending op, the declared footprint, and the violation kind. Check the footprint declaration in your \texttt{RewriteRule} first---most violations are simply missing entries in \texttt{n\_write} or \texttt{e\_write}.
\end{protip}
