...
\begin{watchout}
\textbf{The Zero Invariant}: \texttt{TxId(0)} is reserved as an invalid/sentinel value. Without the \texttt{self.tx\_counter == 0} check, after $2^{64}$ transactions the counter would wrap to zero and confuse code that uses zero to mean ``no transaction.'' Defensive programming at its finest.
\end{watchout}
