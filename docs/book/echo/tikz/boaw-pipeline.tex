...
% NOTE: "BOAW" is a retired codename. The subsystem is now warp_core::parallel.
% Parallel Pipeline Diagram - Standalone TikZ
...
% FootprintGuard (post-hoc check within each worker)
\node[pill, below=5mm of origin] (guard) {\begin{tabular}{@{}c@{}}\textbf{FootprintGuard}\\[-2pt]\scriptsize \texttt{check\_op()} per emitted op\\\scriptsize cfg-gated (debug or opt-in)\end{tabular}};
\draw[dashedarrow] (origin) -- (guard);
% Merge
...
\node[group, fit=(snapshot)(execitems)(w0)(w1)(wk)(origin)(guard)] (g2) {};
