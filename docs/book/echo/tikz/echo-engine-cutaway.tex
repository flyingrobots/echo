...
\coordinate (osC)    at (0,0);     % bottom: JITOS / OS RMG
...
\slabthree{(hubC)}{green!12}{
  ...
  \textbf{Session + RMG Hub:}\\
  session registry, RMG authority,\\
...
% Down: Editor Service → OS (syscalls / RMG rewrites)
\draw[flow] ($(hubC)+(-1.8,-0.9)$) -- node[left=2pt, annot] {%
  syscalls, RMG rewrite\\
