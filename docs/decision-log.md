...
| 2025-12-28 | PR #141 follow-up (SPDX + deny housekeeping) | Align SPDX enforcement with the repo license split by treating build/config files as code (Apache-2.0) and repairing headers accordingly; document required cargo-deny allowlist licenses (OpenSSL, Ubuntu-font-1.0) with explicit dependency justification; move rmg-viewer local crate pins into `[workspace.dependencies]` for DRY version management. | Reduce licensing ambiguity and prevent drift between file headers and crate metadata; keep deny policy explicit and justified; centralize path+version pins to avoid repeated edits across crates. | SPDX CI now enforces the intended split; cargo-deny license policy remains green and understandable; viewer crate updates become a single root bump. Fixes in `042ec2b`, `5086881`, and `17687f2`. |
