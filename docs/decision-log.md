...
| 2025-12-29 | warp-core follow-ups (#151/#152) | Parameterize `Engine.policy_id` (default `POLICY_ID_NO_POLICY_V0`) and add a `GraphStore` reverse edge index to make tick patch replay edge migration/removal `O(bucket_edges)` rather than `O(total_edges)`. | Eliminates “TODO-vibes” by making the policy boundary explicit and ensures patch replay scales to large graphs without full scans. | Callers can select policy id explicitly; patch replay performance improves while preserving determinism. |
