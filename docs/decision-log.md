...
| 2026-01-01 | PR #167 review cleanup (deterministic math) | Tighten determinism invariants surfaced by review: enforce exact odd symmetry for sine by range-reducing `abs(angle)` and applying sign at the end; centralize signed-zero canonicalization via `trig::canonicalize_zero`; remove magic endpoint bits in the LUT generator; add tests asserting the motion executor accepts v0 payloads and upgrades to v2; document deterministic `0/0 → 0` policy; add `// TODO(#115)` tracking for the ignored trig error-budget audit. | Code review found a 1‑ULP mismatch between `sin(±π/8)` golden vectors and flagged duplicated signed-zero handling / implicit invariants. Making symmetry and canonicalization explicit prevents bit drift and reduces the chance of reintroducing nondeterministic edge behavior. | Golden vectors for `sin(-x)` are bit-stable with `sin(x)`; canonicalization logic is centralized and reused by Mat4/Quat; motion v0 support is executable (not just documented); remaining work: convert the ignored audit test into a deterministic oracle once budgets are pinned (#115). |
...
| 2025-12-30 | Branch maintenance: revive `F32Scalar/sin-cos` WIP deterministic trig tests | Merge `main` into the branch (merge commit `6cfa64d`) and port the WIP trig test to `warp-core` paths, keeping the error-budget audit test `#[ignore]` until the deterministic trig backend lands. | The branch predated the `rmg-core`→`warp-core` rename and the test scaffold no longer compiled; syncing now avoids paying the rename tax later when implementing LUT/polynomial-backed trig. | The branch is unblocked and can be finished by implementing deterministic trig for `F32Scalar` and promoting the error-budget scaffold into a real assertion suite. |
