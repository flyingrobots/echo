...
| 2025-12-30 | Branch maintenance: revive `F32Scalar/sin-cos` WIP deterministic trig tests | Merge `main` into the branch (no-commit) and port the WIP trig test to `warp-core` paths, keeping it `#[ignore]` until the deterministic trig backend lands. | The branch predated the `rmg-core`→`warp-core` rename and the test scaffold no longer compiled; syncing now avoids paying the rename tax later when implementing LUT/polynomial-backed trig. | The branch is unblocked and can be finished by implementing deterministic trig for `F32Scalar` and turning the error-budget scaffold into a real assertion suite. |
