...
| ---- | ------- | -------- | --------- | ----------- |
| 2025-12-10 | Docs/dep/UI polish | Updated config README to reference `ConfigStore`; migrated `echo-graph` from `serde_cbor` to maintained `ciborium`; set `echo-session-proto` MSRV to 1.71.1; renamed constellation TikZ asset/graphic; rendered all viewer overlays via exhaustive match (Menu/Settings/Publish/Subscribe). | Align docs with actual trait, avoid unmaintained CBOR crate, keep crate MSRVs consistent with workspace baseline, fix LaTeX asset spelling, and ensure overlays show content instead of silently doing nothing. | README and graphic path are accurate, CBOR serialization uses a maintained crate, proto MSRV matches other crates, LaTeX build uses the correctly named asset, and viewer overlays now cover all variants with explicit UI/close paths. |
| 2025-12-10 | Session client polling | Buffer Unix socket reads until the full 12-byte frame header is read; treat EOF after a partial header as `UnexpectedEof`; added a regression test for partial headers. | Short reads are common on Unix sockets; returning `Ok(None)` after consuming 1–11 bytes dropped the frame start and desynchronized CBOR decoding even though the connection was healthy. | `poll_message` now keeps framing intact (None only on clean EOF before any bytes) and is guarded by a test that writes a split header/payload chunk. |
...
| 2025-12-03 | `rmg-viewer/ROADMAP` | Verified file after VSCode crash; no changes lost. | Git showed clean tree and roadmap matches last commit; no recovery steps required. | Continue roadmap execution from current checklist; no follow-up needed. |
