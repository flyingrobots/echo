...
recovered posture labels. The next sibling jedit fixture should call this
surface through its Echo adapter to classify a submitted edit as not accepted,
accepted pending, decided applied, decided rejected, or obstructed. jedit owns
the mapping from those generic labels to editor language.
