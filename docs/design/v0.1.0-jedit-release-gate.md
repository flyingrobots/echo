...
submission-posture --format json`. jedit may call that command through its
Echo adapter to classify a submitted edit as not accepted, accepted pending,
decided applied, decided rejected, or obstructed. Echo reports only submission
ids, canonical envelope digests, ticket digests, receipt digests, and recovered
posture labels. jedit owns the mapping from those generic labels to editor
language.
