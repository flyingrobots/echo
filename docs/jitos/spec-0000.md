...
```
use serde::{Serialize, Deserialize};
use crate::rmg_core::{Rmg, NodeId, Value, Edge};
...
impl RewriteEngine {
    ...
    pub fn apply(&mut self, rmg: &mut Rmg, rw: Rewrite) {
        match rw.op {
            SemanticOp::AddNode => {
                rmg.nodes.insert(rw.target.clone(), crate::rmg_core::Node {
                    id: rw.target.clone(),
            ...
            SemanticOp::Set => {
                if let Some(node) = rmg.nodes.get_mut(&rw.target) {
                    if let Some(Value::Str(field_name)) = &rw.old_value {
                        node.fields.insert(field_name.clone(), rw.new_value.clone().unwrap());
                    }
                }
            }
...
impl WasmDemo {
    ...
    pub fn serialize_graph(&self) -> JsValue {
        JsValue::from_serde(&self.rmg).unwrap()
    }

    pub fn serialize_history(&self) -> JsValue {
        JsValue::from_serde(&self.engine.history).unwrap()
    }
}
```
...
async function run() {
    ...
    function render() {
        const graph = demo.serialize_graph();
        const history = demo.serialize_history();
        // Render UI (graph + log)
        drawGraph(graph);
        drawLog(history);
    }
...
You’ll fill in drawGraph later with your favorite canvas/SVG lib.

---

# 8. You’re Now Officially Bootstrapped
...
I can produce a clean D3.js/SVG-based layout.

  

### 2. Add reverse-apply for rewrites (time travel slider)

  

Turn your demo into a real debugger.

  

### 3. Add field-level Set & Connect operations

  

Make the first demo more functional.

  

### 4. Add tombstone delete + resurrection

  

Show off lifecycle semantics.

  

### 5. Add SemanticOp-based merge simulation (mini collapse demo)

  

Preview the merge engine in a toy world.
