...
- Executes real WARP logic (Rust -> WASM)
- Serves as the first living test in the OS

---

## 1. Concept: WARP + Rewrite = Reality

In JITOS, the world **is** a WARP graph (WARP).
...
Put this into crates/warp-core/src/lib.rs:
...
use echo_wasm_abi::{Edge, Node, NodeId, Value, WarpGraph};
...
impl RewriteEngine {
    ...
    pub fn apply(&mut self, warp: &mut WarpGraph, rw: Rewrite) {
        match rw.op {
            SemanticOp::AddNode => {
                warp.nodes.insert(rw.target.clone(), Node {
                    id: rw.target.clone(),
                    fields: Default::default(),
                });
            }
            SemanticOp::Set => {
                if let Some(node) = warp.nodes.get_mut(&rw.target) {
            ...
            SemanticOp::DeleteNode => {
                warp.nodes.remove(&rw.target);
                warp.edges.retain(|e| e.from != rw.target && e.to != rw.target);
            }
            SemanticOp::Connect => {
                if let Some(Value::Str(to)) = &rw.new_value {
                    warp.edges.push(Edge {
            ...
            SemanticOp::Disconnect => {
                if let Some(Value::Str(to)) = &rw.new_value {
                    let from = rw.target.as_str();
                    let to = to.as_str();
                    warp.edges.retain(|e| !(e.from == from && e.to == to));
                }
            }
        }
...
impl WasmDemo {
    ...
    pub fn add_node(&mut self, id: String) {
        ...
        self.engine.apply(&mut self.warp, rw);
    }

    pub fn serialize_graph(&self) -> String {
        serde_json::to_string(&self.warp).unwrap_or_default()
    }
