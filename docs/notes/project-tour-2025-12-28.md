...
- The _state_ of the world is a graph (nodes + edges + payloads).
- A _change_ is a rewrite (rule applied at a scope).
- A _frame / tick_ is a transaction:
...
- Empty digests for _length-prefixed list digests_ use `blake3(0u64.to_le_bytes())`.
