...
| `WorldlineTick`   | `crates/echo-runtime-schema/src/lib.rs`   | re-exported by `warp-core`; ABI mirror in `crates/echo-wasm-abi/src/kernel_port.rs`                                   | Shared runtime owner now exists; ABI wrapper must preserve logical-counter semantics without widening runtime feature drift |
| `GlobalTick`      | `crates/echo-runtime-schema/src/lib.rs`   | re-exported by `warp-core`; ABI mirror in `crates/echo-wasm-abi/src/kernel_port.rs`                                   | Shared runtime owner now exists; schema/docs must keep correlation-not-time semantics explicit                              |
...
| `RunId`           | `crates/echo-runtime-schema/src/lib.rs`   | ABI mirror in `crates/echo-wasm-abi/src/kernel_port.rs`; runtime/ABI mapping in `crates/warp-wasm/src/warp_kernel.rs` | Supporting control-plane token needed by `SchedulerStatus`; runtime owner is shared while ABI remains adapter-owned         |
