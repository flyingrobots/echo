...
1. **WARP rewrite scheduler (implemented today)** — lives in Rust `warp-core` and is responsible for:
    ...
    - enforcing _footprint independence_ via `reserve()` (so rewrites commute and ticks are deterministic).
...
| ------------------------------------------------------------------------ | ----------------------- | -------------------------------------------------------------------------------- |
| [scheduler-warp-core.md](scheduler-warp-core.md)                         | **Current**             | WARP rewrite scheduler: `reserve()`, deterministic drain, footprint independence |
| [scheduler-performance-warp-core.md](scheduler-performance-warp-core.md) | **Current**             | Benchmarks and performance analysis for the rewrite scheduler                    |
| [spec-scheduler.md](spec-scheduler.md)                                   | **Not yet implemented** | Future ECS-style system scheduler (systems + phases + dependency DAG)            |
