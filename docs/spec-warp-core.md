...
# WARP Graph Core Specification v2

## 1. Purpose
WARP graph (Worldline Algebra for Recursive Provenance; “WARP”) is a typed, deterministic graph-rewriting engine implemented in Rust. It provides atomic, in-place edits of WARP graphs with deterministic local scheduling and snapshot isolation. WARP is the substrate for the Echo engine: runtime, assets, networking, and tools all operate on the same living graph.
...
- Exportable via `warp snapshot`; can be streamed over network.
...
### Rhai
```rhai
warp.apply("update/transform", #{ entity: id })
```

### TypeScript (WASM)
```ts
await warp.apply("ui/paint");
const graph = await warp.snapshot();
```
...
| Tool | Role |
| --- | --- |
| `warp` CLI | apply rules, emit snapshots, verify confluence |
...
| Phase | Deliverable |
| --- | --- |
| 1.5 | `warp-core` with typed DPOi engine and deterministic scheduler |
...
**TL;DR:** WARP v2 is a deterministic, typed DPOi graph-rewriting engine with atomic in-place updates and confluent distributed synchronization. Echo’s runtime, assets, and tools run atop the same living graph.
