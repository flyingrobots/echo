...
- **Delta patch first:** a tick patch is a _delta_ (canonical edits), not a “recipe” (re-run rule executors).
- **Unversioned slots in the patch:** the patch contains `in_slots` / `out_slots` as _unversioned_ slot identifiers.
