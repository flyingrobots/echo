...
\begin{tourguide}
The post-hoc strategy is a deliberate design choice: we let the executor run to completion (or panic), then inspect what it wrote. This avoids the overhead of intercepting every write call during hot-loop execution. The \texttt{catch\_unwind} boundary means even a panicking rule cannot leave unchecked ops in the delta.
\end{tourguide}
...
explicitly removable for unsafe experimentation. The struct layout changes
depending on the build profile---\texttt{ExecItem} is smaller in release builds
