...
This is a \textbf{Least Significant Digit} radix sort---it processes from the least significant bits to the most significant. After 20 passes (160 bits total), the array is sorted by:
\begin{enumerate}
\item \texttt{scope\_hash} (256 bits, but only 128 bits processed --- the top 128 bits provide enough entropy)
\item then \texttt{rule\_id} (32 bits)
\item then \texttt{nonce} (32 bits)
\end{enumerate}
...
\begin{itemize}
\item 128 bits of scope\_hash = 8 passes
\item 32 bits of rule\_id = 2 passes
\item 32 bits of nonce = 2 passes
\end{itemize}

Wait, that's only 12 passes! The extra 8 passes cover more of the scope\_hash for better distribution. Since LSD radix sort processes from least significant to most significant, passes 4-19 progressively refine the scope ordering.
...
\begin{tourguide}
BOAW---``Bag of Asynchronous Work''---is where Echo's determinism meets parallelism. The key insight: \emph{order of execution doesn't matter if we sort the outputs}. Rules execute in arbitrary order on worker threads, but their outputs are merged canonically.
\end{tourguide}
...
\subsection{5.3 ExecItem Structure}\label{execitem-structure}
...
\subsection{5.4 Thread Safety}\label{thread-safety}
