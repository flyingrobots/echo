...
│
└─ OUTCOME PRECEDENCE (returns Result<TickDelta, PoisonedDelta>):
...
\texttt{Result<TickDelta, PoisonedDelta>}. The API exposes \texttt{merge\_deltas\_ok()}
(replacing \texttt{merge\_deltas()}) which returns \texttt{Result} and only
accepts non-poisoned deltas. A \texttt{PoisonedDelta} cannot be passed to
\texttt{merge\_deltas\_ok()}---the type system prevents accidental merging.
