...
changed `crates/<crate>/src/foo.rs` with inline tests maps to the exact
lib-module witness:
...
Source files without inline tests fall back to the crate check rather than a
fake zero-test module filter:

```sh
cargo check -p <crate> --quiet
```

Binary source edits map to the binary target:

```sh
cargo test -p <crate> --bin <bin_name>
```
...
- `cargo xtask test-slice durability-release` runs the joined WAL/WSC release witness: filesystem runtime WAL durability, WSC retained evidence recovery, app-safe missing-retention posture, recovery plan bootstrap posture, committed-only durability index rebuilds, WSC topology recovery, topology WAL recovery, typed missing-material obstruction, stale-claim guards, doctrine checks, and generated man-page freshness. This is a release-gate slice, not the fastest local edit loop.
