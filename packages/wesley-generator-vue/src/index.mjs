...
function emitClient(ir) {
  ...
  lines.push("  drain_view_ops(): any; // ViewOp[]");
  lines.push("  get_ledger(): any; // (Snapshot, Receipt, Patch)[]");
  lines.push("  jump_to_tick(tickIndex: number): void;");
  lines.push("  render_state(): void;");
  ...
  lines.push("    if (info.codec_id !== CODEC_ID) throw new Error('Codec mismatch');");
      ...
      lines.push("  }");
      lines.push("");
          lines.push("  getLedger() {");
          lines.push("    return this.wasm.get_ledger();");
          lines.push("  }");
          lines.push("");
          lines.push("  jumpToTick(index: number) {");
          lines.push("    this.wasm.jump_to_tick(index);");
          lines.push("  }");
          lines.push("");
          lines.push("  renderState() {");
          lines.push("    this.wasm.render_state();");
          lines.push("  }");
          lines.push("");
          for (const q of queries) {    const fn = `query${pascal(q.name)}`;
