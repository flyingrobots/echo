...
function loadClusterPrefixes() {
  try {
    ...
    if (err?.code !== "ENOENT") {
      console.warn(
        `Failed to read ${CLUSTER_CONFIG_PATH}: ${err.message}; using defaults.`,
      );
    }
  }
...
function wrapLabel(text, maxLineLength = 30) {
  ...
  for (const word of words) {
    ...
    if (
      (current + (current ? " " : "") + word).length > maxLineLength &&
      current.length
    ) {
      lines.push(current);
      current = word;
...
function generateDot(nodes, edges) {
  ...
  for (const [name, groupNodes] of clusters) {
    ...
    for (const node of groupNodes) {
      ...
      lines.push(
        `    i${node.number} [label="${safeLabel}", URL="${escapeDotString(node.url)}", tooltip="${escapeDotString(node.title)}"];`,
      );
    }
    lines.push("  }");
  }
